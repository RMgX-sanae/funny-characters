...
一个会把你输入的文字变成“有规律的胡话”的网页小工具。少笔画生僻字为主，偶尔冒出神秘符号；敲几下键盘，再复制给同学猜猜看。
...
- 每次抽取新字符：**生僻汉字 88%、炼金术符号 10%、楔形文字 1.5%、古埃及象形文字 0.5%**。四类池分别有 144、116、169、96 个字符，共 525 个。
- 中文重复字保持相同：例如“哈哈，哈哈！”会变成类似“㐂㐂，㐂㐂！”的形式。对应关系在本次打开页面期间有效，清空不会重置，刷新页面后重新随机。不同输入字可能对应同一个输出字，不用于加密。
...
- `<main>`：界面。
- `<script>`：分组字符池 `characterGroups`、按权重抽取、中文对应表、输入事件、撤销和复制逻辑。
...
- 完全离线加载，四种字体全部成功载入，525 个字符均有对应字形。
